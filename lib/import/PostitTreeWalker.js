...
function findDisplayCandidate(definitions) {
  return find(definitions.rootElements, function(e) {
    return is(e, 'landscape:landscapeBoard');
  });
}


export default function landscapeTreeWalker(handler, translate) {
  ...
  function handleDefinitions(definitions, rootBoard) {
    ...
    if (rootBoard && rootBoards.indexOf(rootBoard) === -1) {
      throw new Error(translate('rootBoard not part of landscape:Definitions'));
    }
    ...
    if (is(rootElement, 'landscape:landscapeBoard')) {
      handlelandscapeBoard(rootElement, ctx);
    }
  ...
  function handlelandscapeBoard(board, context) {
