...
/**
 * A base viewer for landscape boards.
 *
...
/**
 * Parse and render a landscape diagram.
 *
 ...
 *
 * @param {String} xml the landscape xml
 * @param {ModdleElement<landscapeRootBoard>|String} [rootBoard] landscape board or id of board to render (if not provided, the first one will be rendered)
 *
...
BaseViewer.prototype.importXML = function(xml, rootBoard) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    self._moddle.fromXML(xml, 'landscape:Definitions').then(function(result) {
...
/**
 * Import parsed definitions and render a landscape diagram.
 *
 * Once finished the viewer reports back the result to the
 * provided callback function with (err, warnings).
 *
 ...
 * You can use these events to hook into the life-cycle.
 *
 * @param {ModdleElement<Definitions>} definitions parsed landscape definitions
 * @param {ModdleElement<landscapeRootBoard>|String} [rootBoard] landscape board or id of board to render (if not provided, the first one will be rendered)
 *
 ...
 * You can use these events to hook into the life-cycle.
 *
 * @param {String|ModdleElement<landscapeRootBoard>} [rootBoardOrId] id or the diagram to open
 *
...
BaseViewer.prototype.open = function(rootBoardOrId) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (typeof rootBoardOrId === 'string') {
      ...
      if (!rootBord) {
        var err2 = new Error('landscapeRootBoard <' + rootBoardOrId + '> not found');
    ...
    importlandscapeDiagram(self, definitions, rootBord).then(function(result) {
...
/**
 * Export the currently displayed landscape diagram as
 * a landscape XML document.
 *
 * ## Life-Cycle Events
 *
...
BaseViewer.prototype.clear = function() {
  ...
  // in the landscapeTreeWalker (and assume none are given
  // on reimport)
...
function checkValidationError(err) {
  ...
  // check if we can help the user by indicating wrong landscape xml
  ...
  if (match) {
    err.message =
      ...
      'this may indicate an invalid landscape board file' + match[2];
  }
 ...
 *
 * @return {ModdleElement<landscapeRootBoard>|null}
 */
